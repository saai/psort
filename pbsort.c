/* 
   pbsort.c - Parallel sorting algorithm based on bubblesort
...
int main(int argc, char ** argv)
{
  ...
  s = (n >= c * (id+1)) ? c : (n - c * id);
  // printf("id is %d, n is : %d, c is : %d, step is : %d, s is : %d \n\n", id, n, c, step, s);
  bubblesort(chunk, s);
  ...
  for (step = 1; step < p; step = 2*step) {
    if (id % (2*step)!=0) {
      ...
      // printf("id is %d, step is : %d, s is : %d \n\n", id, step, s);
